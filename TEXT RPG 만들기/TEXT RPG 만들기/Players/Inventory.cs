...
namespace Inventory
{
    public class CInventory
    {
        ...
        public void ShowPotion() // 포션만 따로 모아놓은 목록. 여기서도 문제가 생겨서 여기서 보여준 인덱스 번호랑 아래에서 사용하는 번호가 달랐음.
                                 // 여기서도  포션만 따로 리스트 만들어서 인덱스 1번부터 시작하게 바꿨음.
        {
            Console.WriteLine("======포션 목록======");
            var potions = Items.Where(item => item.ItemKind == ITEM_TYPE.POTION).ToList(); // 이게 포션만 따로 필터링해서 리스트 시키는 코드.
            int index = 1;
            foreach (var potion in potions)
            {
                {
                    Console.WriteLine($"{index}. {potion.Name} - 회복량: {potion.AbilityValue}");
                    index++;
                }
            }
        }
        public void UsingPotion(CPlayer player)
        {
            ...
            if (int.TryParse(Console.ReadLine(), out int selectPotionNum))
            {
                ...
                //Items.Where(item => item.ItemKind == ITEM_TYPE.POTION); //-> 리스트내에서 포션타입만 필터링. 위에서 사용함.
                //ElementAtOrDefault(selectPotionNum - 1); -> 필터링한 내용중에서 선택한 번호에 맞는 리스트를 가져옴. 굳이 필요없는 코드.
                if (selectPotion == null)
                {
                    Console.WriteLine("선택한 번호에 해당하는 포션이 없습니다.");
                    Thread.Sleep(1000);
                    return;
                }
                ...
                if (selectPotion != null)
                {
                    if (selectPotion.Name.Contains("HP"))
                    {
                        ...
                        {
                            Console.WriteLine($"{selectPotion.Name}을 사용해 {player.MaxHp-player.Hp}만큼 회복했습니다");
                    ...
                    if (selectPotion.Name.Contains("MP"))
                    {
                        ...
                        {
                            Console.WriteLine($"{selectPotion.Name}을 사용해 {player.MaxMp-player.Mp}만큼 회복했습니다");
